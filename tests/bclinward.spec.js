...
test('Upload PRD File', async({page})=>{
...
test('Complete putaway process/ Update sublocation', async ({ page }) => {
   ...
   await page.waitForTimeout(3000);
    ...
    await ticketNo.fill("8745934");
    const ticketDate=page.locator("//input[@aria-expanded='undefined']");
    await ticketDate.click();
    const selectDate=page.locator("//div[@data-v-date='2025-10-03']");
  ...
  await OKBtn.click();
    ...
    test.describe('Mobile App Login Test', function () {
     ...
     //console.log("44444444");
        ...
        test('should login successfully with valid credentials00', async () => {
    ...
    test('pending dispatch', async({page})=>{
        test.setTimeout(60000);
        await page.goto('https://rims-demo.blubirch.com/', {timeout: 80000});
        await page.getByRole('textbox', { name: 'Enter email ID/phone number/' }).click();
        await page.getByRole('textbox', { name: 'Enter email ID/phone number/' }).fill('svastiadmin@yopmail.com');
